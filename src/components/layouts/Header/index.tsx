...
const Header = () => {
  ...
  const currentLanguage : currentLanguageProps | undefined  = languages.find(lang => lang.code === i18n.language);
  ...
  const renderFlagLogo = (flagName :string, flagPath: string)=>(
    
      <img
      className="h-8 w-8 rounded-full"
      src={flagPath}
      alt={`${flagName} flag Icon`}
    />
      ...
      <SharedSection className="flex flex-wrap items-center justify-between !py-3">
        ...
        <div className="flex md:order-2 space-x-3 md:space-x-0 rtl:space-x-reverse">
          <div className={`relative ${isRTL ? 'ml-2' : 'mr-2'} dropdown-button`}>
            <button
              className="flex items-center justify-center px-8 py-2 text-gray-500 rounded-lg focus:outline-none focus:ring-2 border-b-2 border-primary-500 focus:border-primary focus:border-b-0"
              onClick={handleDropdownToggle}
            >
              <div className={`${isRTL ? 'ml-2' : 'mr-2'} inline-flex bottom-0`}>
                {(currentLanguage as currentLanguageProps).name}
              </div>
              {renderFlagLogo((currentLanguage as currentLanguageProps).name,(currentLanguage as currentLanguageProps).flag_path)}
 
            </button>
            {isDropdownOpen && (
              <ul className="absolute right-0 mt-2 w-48 bg-white border border-gray-200 rounded-lg shadow-lg">
                {languages.map(({ code, name, country_code, flag_path }) => (
                  <li key={country_code} className="block px-4 py-2 text-gray-700 hover:bg-gray-100">
                    <button
                      className="flex items- w-full text-left justify-between"
                      onClick={() => {
                        i18n.changeLanguage(code);
                        handleDropdownToggle();
                      }}
                    >
                      <span className={`flag-icon flag-icon-${country_code} mr-2`}>{name}</span>
                      {renderFlagLogo(name,flag_path)}
